#include <vector>
#include "FastLPFilter.h"

// Processes a block of audio samples
void FastLPFilter::ProcessBlock(iplug::sample** inputs, iplug::sample** outputs, int nFrames, float cutoffFreq, float sampleRate, int mode, float extraparam) {

  const float velocity = ((1.0 - cutoffFreq) * (1.0 - cutoffFreq));
  ...
  case 0: // Single Smooth
  {
    ...
    for (int s = 0; s < nFrames; s++) {
      for (int c = 0; c < 2; c++) {
        outputs[c][s] = alpha * inputs[c][s] + (1.0f - alpha) * lastOutputs_0[c];
