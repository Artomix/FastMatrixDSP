...
def  main():
  ...
  installer = "\installer\FastMatrix Installer.exe"
   
  if demo:
    installer = "\installer\FastMatrix Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\FastMatrix manual.pdf" 
  ]
  ...
  ZIPNAME = "FastMatrix-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "FastMatrix-v" + FULLVERSIONSTR + "-win-demo.zip"
